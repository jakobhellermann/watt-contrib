...
#[derive(Derivative)]
#[derivative(Debug)]
struct Bar<T, U>(T, #[derivative(Debug = "ignore")] U);
...
fn main() {
    ...
    assert_eq!(
        C::C3::<i32, NoDebug>("foo".to_string()).to_show(),
        "C3(\"foo\")".to_string()
    );
