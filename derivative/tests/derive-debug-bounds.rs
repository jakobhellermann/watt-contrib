...
struct Bar<T, U>(
    T,
    #[derivative(Debug(format_with = "MyDebug::my_fmt", bound = "U: MyDebug"))] U,
);

#[derive(Derivative)]
#[derivative(Debug(bound = "T: std::fmt::Debug, U:MyDebug"))]
struct Bar2<T, U>(T, #[derivative(Debug(format_with = "MyDebug::my_fmt"))] U);
...
#[derive(Derivative)]
#[derivative(Debug)]
struct TestUnneededBound<T>(
    // Test that we don't add T: Debug
...
fn main() {
    assert_eq!(
        ...
        "Foo { foo: 42, bar: MyDebug }".to_string()
    );
    assert_eq!(
        Foo2 { foo: 42, bar: 0 }.to_show(),
        "Foo2 { foo: 42, bar: MyDebug }".to_string()
    );
    assert_eq!(Bar(42, 0).to_show(), "Bar(42, MyDebug)".to_string());
    assert_eq!(Bar2(42, 0).to_show(), "Bar2(42, MyDebug)".to_string());
    assert_eq!(
        TestUnneededBound(GenericNeedsNoDebug(NoDebug)).to_show(),
        "TestUnneededBound(GenericNeedsNoDebug)".to_string()
    );
}
